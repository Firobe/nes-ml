...
module Make (M : Mapper.S) = struct
  ...
  module Mem = struct
    ...
    let get_raw t v = M.read t.mem v
  end
  ...
  module OAM = struct
    ...
    module SM = struct
      type state =
        ...
        | CopyY
        (* read a sprite's Y-coordinate (OAM[n][0], copying it to the
      ...
      type t = { mutable state : state; mutable n : int }

      let create () = { state = CopyY; n = 0 }
    end
    ...
    let decrease_sprite_counters t =
      ...
      for i = 0 to t.last_sprite_this_scanline do
        ...
        if v <> 0u then t.counters.(i) <- U8.(pred v)
      done
    ...
    let pixel t =
      ...
      for i = 0 to t.last_sprite_this_scanline do
        ...
        if (not !found) && t.counters.(i) = 0u then
          ...
          let scroll = 7 in
          let pat =
            U8.(!high $>> scroll $<< 1 $| (!low $>> scroll $& 1u) $& 3u)
          in
          (* opaque pixel *)
          if pat <> 0u then (
            found := true;
  ...
  module Status = struct
    ...
    let create () =
      { sprite_0_hit = false; vblank_enabled = true; vbl_read = false }
  end
  ...
  module Rendering = struct
    ...
    module AT = struct
      ...
      let create () =
        { low = 0u; high = 0u; low_next = false; high_next = false; next = 0u }
    end
  ...
  let set_register t register (v : U8.t) =
    ...
    | 0 ->
        ...
        t.control.sprite_pattern_address <-
          (if nth_bit v 3 then 0x1000U else 0U);
        t.control.background_pattern_address <-
          (if nth_bit v 4 then 0x1000U else 0U);
        ...
        if
          t.control.nmi_enabled && (not old_nmi) && t.status.vblank_enabled
          && not t.status.vbl_read
        then C6502.NMI.pull t.nmi
    | 1 ->
        (* Mask register *)
    ...
    | 6 ->
        ...
        if Mem.read_latch t.memory then
          ...
          t.memory.temp_address <-
            mk_addr ~hi:(get_hi t.memory.temp_address) ~lo:v;
  ...
  let get_register t reg =
    ...
    let res =
      ...
      | 2 ->
          ...
          t.memory.latch <- true;
          let r =
            (int_of_bool t.status.vblank_enabled lsl 7)
            lor (int_of_bool t.status.sprite_0_hit lsl 6)
          in
  ...
  module R = struct
    ...
    let inc_vert old =
      let v = ref old in
      let open U16 in
      if !v $& 0x7000U <> 0x7000U then v := !v + 0x1000U
      else (
        v := !v $& ?~0x7000U;
    ...
    let fetch_next_data t =
      ...
      | 0 ->
          ...
          if r.cycle = 256 then t.memory.address <- inc_vert t.memory.address
      | 1 ->
          if r.cycle <> 0 then (
            (* Reload shifters *)
    ...
    let render_pixel t disp =
      ...
      let pat, pal =
        if t.graphics.background then
          ...
          let pal =
            ((rathigh lsr scroll2) lsl 1)
            lor ((ratlow lsr scroll2) land 1)
            land 3
          in
    ...
    let shift_registers t =
      ...
      r.at.high <-
        (if r.at.high_next then U8.(logor r.at.high 1u) else r.at.high);
    ...
    let data_fetching t disp render =
      let r = t.rendering in
      (* Cycle 0 : IDLE *)
      if r.cycle = 0 then () (* Cycles 1 - 256 : BACKGROUND FETCHING *)
        ...
        if
          ...
          (* v: ....A.. ...BCDEF <- t: ....A.. ...BCDEF *)
          r.cycle = 257 && Graphics.is_rendering t.graphics
        then
          t.memory.address <-
        ...
        fetch_next_data t;
        shift_registers t (* Cycles 337-340 : USELESS *))
      else ()
    ...
    let fetch_sprite t =
      ...
      let fetch_tile_8 ~high =
        ...
        (* Flip vertically *)
        let fine_offset =
          if nth_bit o.latches.(sn) 7 then U16.(7U - fine_offset)
          else fine_offset
        in
      ...
      in
      let fetch =
        if t.control.sprite_size then fetch_tile_16 else fetch_tile_8
      in
      match step with
    ...
    let sprite_fetching t =
      ...
      let o = t.oam in
      (* Cycle 0: IDLE *)
      if r.cycle = 0 then () (* Cycles 1-64: clear OAM' *)
        ...
        sprite_evaluation t (* Cycles 257-320: sprite tile fetching *))
      else if r.cycle <= 320 then (
        o.last_sprite_this_scanline <- (o.next_open_slot / 4) - 1;
        fetch_sprite t (* Cycles 321-340: IDLE *))
      else ()

    let pre_rendering t disp =
      let r = t.rendering in
      (* Clear VBLANK *)
      if r.cycle = 1 then t.status.vblank_enabled <- false;
      ...
      if r.cycle >= 280 && r.cycle <= 304 && Graphics.is_rendering t.graphics
      then
        t.memory.address <-
        ...
        r.frame <- r.frame + 1;
        t.should_render <- Some (Mem.get_raw t.memory 0x3F00U);
    ...
    let increment_cycle t =
      ...
      if r.cycle = 341 then (
        ...
        if r.scanline = 262 then (* End of frame *)
          r.scanline <- 0);
    ...
    let next_cycle t disp =
      ...
      if r.scanline <= 239 then (
        sprite_fetching t;
        data_fetching t disp true (* Post-render scanline : 240  (IDLE) *))
      else if r.scanline = 240 then () (* Vertical blanking *)
        ...
        if t.control.nmi_enabled && not t.status.vbl_read then
          C6502.NMI.pull t.nmi (* Last vertical blanking lines : IDLE *))
      else if r.scanline <= 260 then () (* Pre-rendering scanline *)
  ...
  module Debug = struct
    ...
    let create () =
      {
        names =
          Display.create ~width:64 ~height:62 ~scale:8 ~palette
            "Name tables + palettes";
        attributes =
          Display.create ~width:32 ~height:32 ~scale:16 ~palette:pal_4
            "Attribute tables";
        patterns =
          Display.create ~width:256 ~height:128 ~scale:4 ~palette:pal_4
            "Pattern tables";
        counter = 0;
      }
    ...
    let render_nametables t disp =
      ...
      for x = 0 to 31 do
        let color = Mem.get_raw t.memory U16.(0x3F00U + ?@x) in
      ...
      let set_attr addr x_orig y_orig =
        for y = 0 to 7 do
          for x = 0 to 7 do
            ...
            let v = Mem.get_raw t.memory U16.(?@addr) in
            let x' = (x * 2) + x_orig in
            let y' = (y * 2) + y_orig in
            let open U8 in
      ...
      let set_pattern addr x_orig y_orig =
        for tile_y = 0 to 15 do
          for tile_x = 0 to 15 do
            ...
            for y = 0 to 7 do
              for x = 0 to 7 do
                let low = Mem.get_raw t.memory U16.(?@addr + ?@y) in
                let high = Mem.get_raw t.memory U16.(?@addr + ?@y + 8U) in
                let shift = 7 - x in
                let open U8 in
